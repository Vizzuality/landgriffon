...
const schemeBluYl = ['#f7feae', '#b7e6a5', '#7ccba2', '#46aea0', '#089099', '#00718b', '#045275'];

const schemeEmrld = ['#d3f2a3', '#97e196', '#6cc08b', '#4c9b82', '#217a79', '#105965', '#074050'];
...
export function getFlowMapColors(settingsState: SettingsState): Colors | DiffColors {
  return getColors(
    false, // TODO: diffMode
...
function getFlowAndCircleColors(
  ...
  const flowColorScheme =
    (inputColors && inputColors.flows && inputColors.flows.scheme) || defaultFlowColorScheme;
...
function getBaseColorsRGBA(colors: Colors | DiffColors | undefined): BaseColorsRGBA {
  ...
  return {
    ...
    dimmedOpacity:
      colors && colors.dimmedOpacity != null ? colors.dimmedOpacity : DEFAULT_DIMMED_OPACITY,
  };
}
...
export function mixColorsRGBA(color1: RGBA, color2: RGBA, zeroToOne: number): RGBA {
