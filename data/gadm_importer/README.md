# GADM importer <!-- omit from toc -->

- [1. `geo_region`](#1-geo_region)
- [2. `gadm_levels0_2`](#2-gadm_levels0_2)
- [3. `admin_region`](#3-admin_region)

---

This importer populates tables `geo_region`, `admin_region` and from precomputed data stored in s3 bucket.

## 1. `geo_region`

`geo_region` consists in gadm levels `0`, `1` and `2` with the geometries converted to h3 compacted and flat. It takes a
while to compute, so it is stored in s3 bucket. To recreate/update it, go to [`preprocessing/gadm/`](../preprocessing/gadm/)
and follow the instructions in [`Makefile`](../preprocessing/gadm/Makefile).
